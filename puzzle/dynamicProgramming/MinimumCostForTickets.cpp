...
// in advance.  The days of the year that you will travel is given as an array days.  Each 
...
public:
    int mincostTickets(vector<int>& days, vector<int>& costs) {
        ...
        vector<int> result(last_day + 1);
        unordered_set<int> travel(days.begin(), days.end());
        for (int i = 1; i <= last_day; ++i) {
            if (travel.count(i) == 0) {
                result[i] = result[i - 1];
            } else {
                result[i] = std::min({result[std::max(0, i - 1)] + costs[0],
                                     result[std::max(0, i - 7)] + costs[1],
                                     result[std::max(0, i - 30)] + costs[2]});
            }
        }
